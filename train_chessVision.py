...
def get_args_parser():
    ...
    parser.add_argument(
        ...
        type=Path,
        default="/root/chess-hackathon-4/model_config.yaml",
    )
    parser.add_argument(
        "--save-dir",
        help="save checkpoint path",
        type=Path,
        default=os.getenv("OUTPUT_PATH", ".logs"),
    )
    parser.add_argument(
        "--load-path", help="path to checkpoint.pt file to resume from", type=Path
    )
    ...
    parser.add_argument(
        "--grad-accum", help="gradient accumulation steps", type=int, default=10
    )
    parser.add_argument(
        "--save-steps", help="saving interval steps", type=int, default=500
    )
    parser.add_argument(
        "--distributed", action="store_true", help="whether to use distributed training"
    )
    parser.add_argument(
        "--patience", help="patience of early stopping", type=int, default=5
    )
    parser.add_argument(
        "--amp", action="store_true", help="Mixed precision"
    )
    parser.add_argument(
        "--mode", help="run mode: train or test", type=str, default="train"
    )
    return parser
...
def test_model(model, test_dataloader, device, amp):
    """
    Run the model in evaluation mode on test/validation data without loss.
    """
    model.eval()
    total_rank_corr = 0.0
    ...
    with torch.no_grad():
        ...
        for batch_idx, (boards, scores) in progress_bar:
            boards, scores = (
            ...
            running_avg_rank_corr = total_rank_corr / (batch_idx + 1)

            progress_bar.set_postfix(
                {"Batch": (batch_idx + 1), "Rank Corr": f"{running_avg_rank_corr:.4f}"}
            )

    avg_rank_corr = total_rank_corr / len(test_dataloader)

    print(f"Spearman Rank Correlation: {avg_rank_corr:.4f}")
...
def main(args, timer):
    ...
    if args.distributed:
        ...
        world_size = dist.get_world_size()  # Total number of GPUs in the cluster
        ...
        )  # Use the first available GPU or CPU
        args.is_master = True

    device = torch.device(f"cuda:{args.device_id}" if args.device_id >= 0 else "cpu")

    if args.is_master and args.save_dir and args.mode == "train":
        writer = SummaryWriter(log_dir=args.save_dir / "logs")

    if args.device_id >= 0 and args.mode == "train":
    ...
    if args.distributed:
        ...
        print(
            f"Number of workers per GPU: {num_workers_per_gpu}, Total workers: {num_workers_per_gpu * num_gpus}"
        )
    ...
    if args.mode == "train":
        if args.distributed:
            model = DDP(
                model, device_ids=[args.device_id], find_unused_parameters=False
            )
        ...
        optimizer = Lookahead(optimizer, k=5, alpha=0.5)  # Lookahead wrapper
        ...
        elif os.path.islink(local_resume_path):
            checkpoint_path = os.path.join(
                os.readlink(local_resume_path), "checkpoint.pt"
            )
        ...
        if checkpoint_path:
            ...
            checkpoint = torch.load(
                checkpoint_path, map_location=f"cuda:{args.device_id}"
            )
            ...
            if "scheduler" in checkpoint:
                ...
                print(
                    "Scheduler state not found in checkpoint. Creating new scheduler."
                )
            else:
        ...
        for epoch in range(train_dataloader.sampler.epoch, MAX_EPOCHS):
            with train_dataloader.sampler.in_epoch(epoch):
                ...
                train_steps_per_epoch = math.ceil(
                    train_batches_per_epoch / args.grad_accum
                )
                optimizer.zero_grad()
                model.train()
                ...
                for boards, scores in train_dataloader:
                    ...
                    batch = (
                        train_dataloader.sampler.progress // train_dataloader.batch_size
                    )
                    ...
                    boards, scores = (
                        boards.to(args.device_id, non_blocking=True),
                        scores.to(args.device_id, non_blocking=True),
                    )
    ...
    Avg Loss [{avg_loss:,.3f}], Rank Corr.: [{rpt_rank_corr:,.3f}%], Examples: {rpt['examples_seen']:,.0f}"""
                        ...
                        if args.is_master:
                            ...
                            writer.add_scalar(
                                "Train/Rank_Corr", rpt_rank_corr, global_step
                            )
                    ...
                    with torch.no_grad():
                        for boards, scores in test_dataloader:
                            ...
                            if is_last_batch:
                                ...
                                avg_loss = rpt["accum_loss"] / rpt["examples_seen"]
                                rpt_rank_corr = (
                                    100 * rpt["rank_corr"] / rpt["examples_seen"]
                                )
                                ...
                                if args.is_master:
                                    ...
                                    writer.add_scalar(
                                        "Validation/Rank_Corr", rpt_rank_corr, epoch
                                    )
        ...
        # Test mode: just load the model and run it on test data
        checkpoint = torch.load(args.load_path, map_location=device)
        model.load_state_dict(checkpoint["model"])
        test_model(model, test_dataloader, device, args.amp)
