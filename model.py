...
def encode_board(board: Board) -> np.array:
    ...
    return np.array([PIECE_CHARS[::step].index(c) for c in unicode], dtype=int).reshape(
        8, 8
    )
...
class Residual(nn.Module):
    ...
    def __init__(
        self, outer_channels, inner_channels, use_1x1conv, dropout, dilation=1
    ):
        super().__init__()
    ...
    def __init__(
        ...
        for i in range(nlayers):
            self.convLayers.append(
                Residual(
                    self.embed_dim, self.inner_dim, self.use_1x1conv, self.dropout, 2**i
                )
            )
